  ...
  try {
    ...
    if (ResultCode === 0) {
      ...
      if (order) {
        ...
        console.log(
          `   Warning: Order not found for CheckoutRequestID: ${CheckoutRequestID}`,
        );
      }
    } else {
...
export const queryPaymentStatus = async (req, res) => {
  try {
    ...
    const errorMessage = error.message || "Failed to query payment status";
    const isAuthError =
      errorMessage.includes("access token") || errorMessage.includes("auth");
