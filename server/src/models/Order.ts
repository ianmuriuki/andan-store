...
orderSchema.methods.calculateTotals = function(): void {
  this.itemsPrice = this.items.reduce((total: number, item: IOrderItem) => {
...
orderSchema.methods.canBeRefunded = function(): boolean {
  return ['delivered'].includes(this.status) && this.paymentInfo.status === 'completed';
};
...
orderSchema.pre('save', function(next) {
  ...
  if (this.isNew) {
    const deliveryDays = this.shippingAddress.city.toLowerCase() === 'nairobi' ? 2 : 5;
